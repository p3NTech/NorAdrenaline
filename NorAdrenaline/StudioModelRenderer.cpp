	...
	if (!cvar.hide_from_obs && cvar.esp && !g_pGlobals.bSnapshot && !g_pGlobals.bScreenshot)
	{
		if (g_Utils.IsPlayer(ent) && ent->index != g_Local.iIndex)
		{
			...
			if (cvar.glow_players)
			{
				...
				if (cvar.glow_type == 1)
				{
					g_pStudio->SetForceFaceFlags(STUDIO_HAS_NORMALS);
				}
				else if (cvar.glow_type == 2)
				{
					g_pStudio->SetForceFaceFlags(STUDIO_HAS_VERTICES);
				}
				else if (cvar.glow_type == 3)
				{
					g_pStudio->SetForceFaceFlags(STUDIO_HAS_BBOX);
				}
				else if (cvar.glow_type == 4)
				{
					g_pStudio->SetForceFaceFlags(STUDIO_HAS_CHROME);
				}
				else if (cvar.glow_type == 5)
				{
					g_pStudio->SetForceFaceFlags(STUDIO_NF_FLATSHADE);
				}
				else if (cvar.glow_type == 6)
				{
